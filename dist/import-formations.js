...
async function importFormations(filePath) {
    ...
    try {
        ...
        const universitiesRaw = XLSX.utils.sheet_to_json(universitiesSheet, {
            header: 0,
        });
        ...
        const universitiesMap = new Map(); // nameFr -> id
        ...
        const establishmentsRaw = XLSX.utils.sheet_to_json(establishmentsSheet, { header: 0 });
        ...
        const establishmentsMap = new Map(); // nameAr -> id
        let createdCount = 0;
        let skippedCount = 0;
        for (const etab of establishmentsData) {
            const universityId = universitiesMap.get(etab.universityNameFr);
            if (!universityId) {
                console.warn(`  ⚠️  University not found for establishment: ${etab.nameFr}`);
                skippedCount++;
                continue;
            }
        ...
        const programsData = [];
        const programsRaw = XLSX.utils.sheet_to_json(programsSheet, {
            header: 0,
        });
        ...
        for (const prog of programsData) {
            ...
            if (!establishmentId) {
                console.warn(`  ⚠️  Establishment not found for program: ${prog.nameFr}`);
        ...
        console.log(`  Summary: Created ${programsCreatedCount}, Skipped ${programsSkippedCount}`);
        // Final statistics
        console.log('\n📊 Import Summary:');
...
// Run the import
const excelFilePath = process.argv[2] || './offres de formations.xlsx';
